...
class Crain {
    public static int solution(int[][] board, int[] moves) {
        int answer = 0;
        int length = moves.length; // 전체 길이 
        int numberArray = board[0].length; // N의 수 
        int j = 0; // 2차원 배열 순서 
        ArrayList<Integer> list = new ArrayList<Integer>();
       
        for (int a = 0; a < length; a++) { // 입력 받은 moves 가 끝날 때 까지 
        	j = moves[a];
        	for(int i = 0; i < numberArray; i++) { // 한 배열 씩 검사 
      
        		if (board[i][j - 1] != 0) { // 0 이 아니라면 
            		list.add(board[i][j - 1]); // 리스트에 추가해줌 
      
            		board[i][j - 1] = 0; // 가져온 값을 0 으로 바꿔줌 
                  
                    break;
        			
            	}
        		else {
        			if (i == numberArray - 1) { // 해당 배열에 0만 담겨있다면 
        				break;
        			}
        		}
        	}
